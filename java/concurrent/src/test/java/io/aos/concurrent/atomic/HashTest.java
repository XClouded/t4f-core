...
public class HashTest extends BaseTest implements Runnable {
  static int nLoops;
  static int nThreads;
  ...
  public static void main(String... args) {
    ...
    doTest(new Hashtable());
    hashNTS.stop();
    System.out.println("Standard hashtable (" + nThreads + " threads) took "
        + hashNTS);
    ...
    doTest(new ConcurrentHashMap());
    concNTS.stop();
    System.out.println("Concurrent hashmap (" + nThreads + " threads) took "
        + concNTS);

    double d = ((double) (hashtableTS.elapsedTime() - hashTS.elapsedTime()))
        / nLoops;
    System.out.println("Hashmap saves " + d + " " + hashTS.units()
        + " per call vs. hashtable");

    d = ((double) (hashTS.elapsedTime() - concTS.elapsedTime())) / nLoops;
    System.out.println("Unsynchronized operation (1 thread) saves " + d + " "
        + concTS.units() + " per call");

    d = ((double) (hashNTS.elapsedTime() - concNTS.elapsedTime()))
        / (nLoops * nThreads);
    System.out.println("Unsynchronized operation (" + nThreads
        + " threads) saves " + d + " " + concNTS.units() + " per call");
  }
