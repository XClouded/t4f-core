 ...
 * under the License.                                           *
 ****************************************************************/
package io.aos.concurrent.deadlock;

import io.aos.concurrent.deadlock.DeadlockDetectedException;
 ...
 * for everyday usage. The purpose of this class is to test for deadlocks. The
 * lock() method now throws a DeadlockDetectedException, if a deadlock occurs.
 */
public class DeadlockDetectingLockTest extends ReentrantLock {
    private boolean debugging = true;

    public DeadlockDetectingLockTest(boolean fair, boolean debug) {
        super(fair);
        debugging = debug;
        registerLock(this);
    }
...
//    @Test
    ...
    private static synchronized void registerLock(DeadlockDetectingLockTest ddl) {
        if (!deadlockLocksRegistry.contains(ddl))
            deadlockLocksRegistry.add(ddl);
    }

    private static synchronized void unregisterLock(DeadlockDetectingLockTest ddl) {
        if (deadlockLocksRegistry.contains(ddl))
    ...
    private static Iterator getAllLocksOwned(Thread t) {
        DeadlockDetectingLockTest current;
        ArrayList results = new ArrayList();

        Iterator itr = deadlockLocksRegistry.iterator();
        while (itr.hasNext()) {
            current = (DeadlockDetectingLockTest) itr.next();
    ...
    // Must own class lock prior to calling this method
    private static Iterator getAllThreadsHardwaiting(DeadlockDetectingLockTest l) {
    ...
    private static synchronized boolean canThreadWaitOnLock(Thread t,
            DeadlockDetectingLockTest l) {
        Iterator locksOwned = getAllLocksOwned(t);
        while (locksOwned.hasNext()) {
            DeadlockDetectingLockTest current = (DeadlockDetectingLockTest) locksOwned
                    .next();
    ...
    private static Lock a = new DeadlockDetectingLockTest(false, true);
    private static Lock b = new DeadlockDetectingLockTest(false, true);
    private static Lock c = new DeadlockDetectingLockTest(false, true);
