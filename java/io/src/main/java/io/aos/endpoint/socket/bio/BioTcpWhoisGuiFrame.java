...
public class BioTcpWhoisGuiFrame extends JFrame {
  ...
  private BioTcpWhoisServer server;
  
  public BioTcpWhoisGuiFrame(BioTcpWhoisServer whois) {
    ...
    chosenServer.addActionListener( new ActionListener() {
      public void actionPerformed(ActionEvent evt) { 
        try {
          ...
          BioTcpWhoisServer newServer = new BioTcpWhoisServer(newHost);
          server = newServer;  
        }
        catch (Exception e) {
  ...
  class LookupNames implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evt) {
      ...
      BioTcpWhoisServer.SearchIn group = BioTcpWhoisServer.SearchIn.ALL;
      BioTcpWhoisServer.SearchFor category = BioTcpWhoisServer.SearchFor.ANY;
      
      String searchForLabel = searchFor.getSelection().getActionCommand();
      String searchInLabel = searchIn.getSelection().getActionCommand();
      if (searchInLabel.equals("Name")) group = BioTcpWhoisServer.SearchIn.NAME; 
      else if (searchInLabel.equals("Mailbox")) {
        group = BioTcpWhoisServer.SearchIn.MAILBOX;
      }
      else if (searchInLabel.equals("Handle")) {
        group = BioTcpWhoisServer.SearchIn.HANDLE;
      }

      if (searchForLabel.equals("Network")) {
        category = BioTcpWhoisServer.SearchFor.NETWORK;
      }
      else if (searchForLabel.equals("Person")) {
        category = BioTcpWhoisServer.SearchFor.PERSON;
      }
      else if (searchForLabel.equals("Host")) {
        category = BioTcpWhoisServer.SearchFor.HOST;
      }
      else if (searchForLabel.equals("Domain")) {
        category = BioTcpWhoisServer.SearchFor.DOMAIN;
      }
      else if (searchForLabel.equals("Organization")) {
        category = BioTcpWhoisServer.SearchFor.ORGANIZATION;
      }
      else if (searchForLabel.equals("Group")) {
        category = BioTcpWhoisServer.SearchFor.GROUP;
      }
      else if (searchForLabel.equals("Gateway")) {
        category = BioTcpWhoisServer.SearchFor.GATEWAY;
      }
      else if (searchForLabel.equals("ASN")) {
        category = BioTcpWhoisServer.SearchFor.ASN;
      }
  ...
  public static void main(String... args) {
    ...
    try {
      BioTcpWhoisServer server = new BioTcpWhoisServer();
      BioTcpWhoisGuiFrame a = new BioTcpWhoisGuiFrame(server);
      ...
      System.err.println("Error: Could not locate default host "
       + BioTcpWhoisServer.DEFAULT_HOST);
