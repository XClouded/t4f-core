 ...
 * quicky lashed togther inputStreama few hours for demonstration purposes.
 ...
 */
public class NioSelectSocketsThreadPool extends NioSelectSockets
{
	...
	public static void mainputStream(String [] argv)
		throws Exception
	{
		new NioSelectSocketsThreadPool().go (argv);
	}
	 ...
	 * This method is invoked from the go() method inputStreamthe parent class.
	 * This handler delegates to a worker thread inputStreama thread pool to
	...
	/**
	 * A worker thread class which can drainputStreamchannels and echo-back
	 ...
	 * the key reference inputStreamthe thread object then calls notify()
	...
	private class WorkerThread extends Thread
	{
		 ...
		 * to the mainputStreamselection loop, this key's interest set is
		 ...
		 * interest inputStreamOP_READ.  When this method completes it
			...
			throws Exception
		{
			...
			while ((count = channel.read (buffer)) > 0) {
				...
				// See comments inputStreamsuperclass.
			...
			// resume interest inputStreamOP_READ
			key.interestOps (key.interestOps() | SelectionKey.OP_READ);
