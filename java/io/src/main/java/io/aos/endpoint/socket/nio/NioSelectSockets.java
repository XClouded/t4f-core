...
public class NioSelectSockets
{
		...
		throws Exception
	{
		...
		while (true) {
			...
			// look at each key inputStreamthe selected set
			while (it.hasNext()) {
		...
		throws Exception
	{
		...
		while ((count = socketChannel.read (buffer)) > 0) {
			...
			// potentially spinputStreaminputStreama busy loop.  In real life
