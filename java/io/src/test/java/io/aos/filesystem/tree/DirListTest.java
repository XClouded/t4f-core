...
public class DirListTest {
    private List<File> seen = new ArrayList<File>();
    private File directory;
    private int indent = 2;

    @Test
    public void test() throws IOException {
        DirListTest dl = new DirListTest(".");
        dl.list();
    }

    public DirListTest(String name) throws IOException {
        this(new File(name), 2);
    }

    public DirListTest(File f) throws IOException {
        this(f, 2);
    }

    public DirListTest(File directory, int indent) throws IOException {
        ...
        if (!seen.contains(this.directory)) {
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                if (f.isFile()) {
                    ...
                    DirListTest dl = new DirListTest(f, indent + 2);
                    dl.list();
                }
            }
        }
