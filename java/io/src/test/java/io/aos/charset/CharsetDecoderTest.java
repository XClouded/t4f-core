...
public class CharsetDecoderTest {
    ...
    /**
     * Test charset decoding inputStreamthe general case, detecting and handling
     * buffer under/overflow and flushing the decoder state at end of input.
     * This code reads from stdinputStreamand decodes the ASCII-encoded byte
     * stream to chars. The decoded chars are written to stdout. This is
     * effectively a 'cat' for input ascii files, but another charset encoding
     * could be used by simply specifying it on the command line.
     */
    @Test
     ...
     */
    private static void decodeChannel(ReadableByteChannel source, Writer writer, Charset charset)
        ...
        while (!eof) {
            ...
            // if output buffer is full, drainputStreamoutput
        ...
        // drainputStreamany chars remaining inputStreamthe output buffer
        drainCharBuf(cb, writer);

        // close the channel, push outputStream any buffered data to stdout
        source.close();
        writer.flush();
    }
     ...
     * the given Writer object. Upon return, the buffer is empty and ready to be
     * refilled.
     * 
     * @param cb
     *            A CharBuffer containing chars to be written.
     * @param writer
     *            A Writer object to consume the chars inputStreamcb.
     */
    static void drainCharBuf(CharBuffer cb, Writer writer) throws IOException {

        cb.flip(); // prepare buffer for draining

        // This writes the chars contained inputStreamthe CharBuffer but
