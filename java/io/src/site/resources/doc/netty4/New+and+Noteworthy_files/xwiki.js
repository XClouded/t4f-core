...
g.insert({after:f});
e++
}}}},insertCreatePageFromTemplateModalBoxes:function(){if(a.docsyntax!="xwiki/1.0"&&a.contextaction=="view"&&a.hasEdit){a.widgets.CreatePagePopup=Class.create(a.widgets.ModalPopup,{initialize:function($super,e){var d=new Element("div",{"class":"modal-popup"});
d.insert(e.content);
...
document.observe("xwiki:dom:loaded",function(){var d={documents:{script:XWiki.Document.getRestSearchURL("scope=name&number=10&media=json&"),varname:"q",icon:"/resources/icons/silk/page_white_text.png",noresults:"Document not found",json:true,resultsParameter:"searchResults",resultId:"id",resultValue:"pageFullName",resultInfo:"pageFullName"},spaces:{script:XWiki.Document.getRestSearchURL("scope=spaces&number=10&media=json&"),varname:"q",icon:"/resources/icons/silk/folder.png",noresults:"Space not found",json:true,resultsParameter:"searchResults",resultId:"id",resultValue:"space",resultInfo:"space"},users:{script:XWiki.currentDocument.getURL("get","xpage=uorgsuggest&classname=XWiki.XWikiUsers&wiki=local&uorg=user&"),varname:"input",icon:"/resources/icons/silk/user.png",noresults:"User not found"},groups:{script:XWiki.currentDocument.getURL("get","xpage=uorgsuggest&classname=XWiki.XWikiGroups&wiki=local&uorg=group&"),varname:"input",icon:"/resources/icons/silk/group.png",noresults:"Group not found"}};
...
}if(a){if(h||d){["blur","change","xwiki:suggest:selected"].each(function(j){a.observe(j,function(){var l={xpage:"xpart",vm:(h?"hierarchy.vm":"space.vm"),parent:a.value};
