 ...
 * 
 * S - Start Game Move - Aim Crosshairs
 * P - Toggle Pause Button - Start Game/Fire 
 * M - Toggle Sound
 * 
 */
public class SpaceShotMain extends Applet implements Runnable, KeyListener, MouseListener, MouseMotionListener {
    ...
    int[] typedmg = { 2, 4, 5, 3 };
    String[] typenme = { "F15 Fighter", "F17A Advanced Tactical Fighter",
            "F21 Fighter/Bomber", "SF-111 Interceptor" };
    ...
    FontMetrics smallFm = getFontMetrics(smallFont);
    ...
    public void init() {
        ...
        // Set up mouse and key event handling and set focus to the applet
        // window.
    ...
    public void run() {
        ...
        while (Thread.currentThread() == loopThread) {
            ...
            // In playing or ending mode, update moving objects and advance
            // blast
            // counter if active.
            ...
            // In game ending mode, advance ending counter and start demo when
            // it
            // hits zero.
    ...
    public void initStar(int i) {
        ...
        starsRadius[i] = Math.max(scrnWidth, scrnHeight) / 2 + Math.random()
                * Math.min(scrnWidth, scrnHeight) / 4;
    ...
    public void updateStars() {
        ...
        for (i = 0; i < NUM_STARS; i++) {
            if ((starsRadius[i] *= .95) < starsMinRadius[i])
                initStar(i);
            starsX[i] = (int) (scrnWidth / 2 + starsRadius[i]
                    * Math.sin(starsAngle[i]));
            starsY[i] = (int) (scrnHeight / 2 + starsRadius[i]
                    * Math.cos(starsAngle[i]));
        }
    }
    ...
    public void updateShips() {
        ...
        for (i = 0; i < NUM_SHIPS; i++) {
            ...
            // If exploding, decrement explosion counter. Generate new ship at
            // end.
            ...
            // Otherwise, move the ship.
                ...
                ships[i].dist1 = Math.sqrt(Math.pow(ships[i].dx, 2)
                        + Math.pow(ships[i].dy, 2) + Math.pow(ships[i].dz, 2));
                ...
                // If too close, let it take a shot then remove and generate a
                // new
                // ship.
    ...
    public void explodeShip(int i) {
        ...
        for (j = 0; j < ships[i].faces.size(); j += 2) {
            ...
            debris[debrisIndex].mx = Math.random() * ships[i].speed / 2
                    - ships[i].speed / 4;
            debris[debrisIndex].my = Math.random() * ships[i].speed / 2
                    - ships[i].speed / 4;
            debris[debrisIndex].mz = ships[i].speed + Math.random()
                    * ships[i].speed / 2 - ships[i].speed / 4;
        ...
        for (i = 0; i < MAX_DEBRIS; i++)
            if (debris[i].active) {
                ...
                if (debris[i].dz > END_DISTANCE) {
                    if (sound && Math.abs(debris[i].dx) < MAG / 2
                            && Math.abs(debris[i].dy) < MAG / 2)
                        debrisSound.play();
                    debris[i].active = false;
                } else
                    debris[i].render(viewpoint, MAG, scrnWidth / 2,
                            scrnHeight / 2);
    ...
    public void keyPressed(KeyEvent e) {
        ...
        // Allow upper or lower case characters for keys.
    ...
    public void paint(Graphics g) {
        ...
        for (i = 0; i < TGObject.list.size(); i++) {
            ...
            for (j = 0; j < shape.polygons.size(); j++) {
                ...
                // If this is a ship, is it targeted and are we firing? (If
                // ships
                // overlap, only the last, or nearest, should be hit so save the
                // index.)

                if (!(shape instanceof TGDebris) && polygon.contains(mx, my)) {
                    targeted = true;
        ...
        if (targeted && gameState != END) {
            ...
            s = speed + "mps";
            offGraphics.drawString(s, mx - x
                    - (smallFm.stringWidth(s) + smallFontWidth), my);
        }
        ...
        if (gameState == DEMO) {
            ...
            if (!loaded) {
                ...
                if (clipTotal > 0)
                    offGraphics.fillRect(x, y,
                            (int) (w * clipsLoaded / clipTotal), h);
                offGraphics.setColor(Color.white);
                offGraphics.drawRect(x, y, w, h);
                offGraphics.drawString(s, x + 2 * fontWidth,
                        y + fm.getMaxAscent());
            }
            ...
            if (targeted) {
                ...
                offGraphics.drawString(s,
                        (d.width - smallFm.stringWidth(s)) / 2, 6 * fontHeight);
            }
            offGraphics.setColor(Color.red);
            s = typenme[demoIndex];
            offGraphics.drawString(s, d.width / 2
                    - (smallFm.stringWidth(s) + fontWidth), d.height - 4
                    * fontHeight);
            s = "Maximum speed: "
                    + (MAX_SPEED * typespd[demoIndex] * 1000 / DELAY) + "mps";
            offGraphics.drawString(s, d.width / 2 + smallFontWidth, d.height
                    - 4 * fontHeight);
            s = "Weapon strength: " + typedmg[demoIndex];
            offGraphics.drawString(s, d.width / 2
                    - (smallFm.stringWidth(s) + fontWidth), d.height - 3
                    * fontHeight);
...
class TGPoint {
     ...
     * @param dz
     */
    public void transform(double ax, double ay, double az, double m, double dx,
            double dy, double dz) {
        double x, y, z, c, s;
 ...
 * each visible face. These should be used for drawing the object. There is also
 * a master list of all TGObjects that have been rendered. The objects should be
 * drawn in the order given by this list so that more distant objects are drawn
 * before closer ones.
 ******************************************************************************/
...
class TGObject {
    ...
    // Render each face by projecting its points and creating a two-dimensional
    // polygon.
    ...
    public void render(TGPoint v, double mag, int xoffset, int yoffset) {
        ...
        for (i = 0; i < this.vertices.size(); i++) {
            ...
            point.transform(this.ax, this.ay, this.az, this.m, this.dx,
                    this.dy, this.dz);
            point.project(v);
        }
        ...
        for (i = 0; i < this.faces.size(); i++) {
            ...
            for (j = 0; j < face.indices.size(); j++) {
                ...
                polygon.addPoint(
                        (int) Math.round((mag * point.xp) + xoffset),
                        (2 * yoffset)
                                - Math.round((int) (mag * point.yp) + yoffset));
            ...
            // Find the angle of the face with respect to the viewpoint. Only
            // add the
            // polygon if the outside of the face is visible (right-hand rule).
    ...
    // Insert a three-dimensional object into the master list according to
    // depth.
