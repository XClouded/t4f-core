...
public class AsteroidsApplet extends Applet implements Runnable, KeyListener {
    private static final long serialVersionUID = 1L;

    protected static final int INITIAL_WIDTH = 400;
    protected static final int INITIAL_HEIGHT = 400;

    private String copyName = "Asteroids";
    private String copyVers = "Version 0.0.1";
    private String copyInfo = "Copyright AOS";
    private String copyLink = "http://aos.io";
    ...
    private static final int FPS = Math.round(1000 / DELAY); // the resulting
                                                             // frame rate.
    ...
    private static final int SCRAP_COUNT = 2 * FPS; // Timer counter starting
                                                    // values
    private static final int HYPER_COUNT = 3 * FPS; // calculated using number
                                                    // of
    private static final int MISSLE_COUNT = 4 * FPS; // seconds x frames per
                                                     // second.
    private static final int STORM_PAUSE = 2 * FPS;

    private static final int MIN_ROCK_SIDES = 6; // Ranges for asteroid shape,
                                                 // size
    ...
    // Sprite objects.
    private AsteroidsSprite ship;
    private AsteroidsSprite fwdThruster, revThruster;
    private AsteroidsSprite ufo;
    private AsteroidsSprite missle;
    private AsteroidsSprite[] photons = new AsteroidsSprite[MAX_SHOTS];
    ...
    // Ship data.
    private int shipsLeft; // Number of ships left in game, including current
                           // one.
    ...
    private int ufoCounter; // Timer counter used to track each flying saucer
                            // pass.
    ...
    // Asteroid data.
    private boolean[] asteroidIsSmall = new boolean[MAX_ROCKS]; // Asteroid size
                                                                // flag.
    ...
    // Explosion data.
    private int[] explosionCounter = new int[MAX_SCRAP]; // Time counters for
    // explosions.
    ...
    public void init() {
        ...
        // Dimension d = getSize();
        AsteroidsSprite.width = INITIAL_WIDTH;
        // AsteroidsSprite.width = d.width;
        AsteroidsSprite.height = INITIAL_HEIGHT;
    ...
    public void paint(Graphics g) {
        ...
        if (ufo.active) {
            ...
            offGraphics.drawLine(ufo.sprite.xpoints[ufo.sprite.npoints - 1],
                    ufo.sprite.ypoints[ufo.sprite.npoints - 1], ufo.sprite.xpoints[0], ufo.sprite.ypoints[0]);
        }
    ...
    private void loadSounds() {
        ...
        crashSound = getAudioClip(Thread.currentThread().getContextClassLoader().getResource("crash.au"));
        clipTotal++;
        explosionSound = getAudioClip(Thread.currentThread().getContextClassLoader().getResource("explosion.au"));
        clipTotal++;
        fireSound = getAudioClip(Thread.currentThread().getContextClassLoader().getResource("fire.au"));
        clipTotal++;
        missleSound = getAudioClip(Thread.currentThread().getContextClassLoader().getResource("missle.au"));
        clipTotal++;
        saucerSound = getAudioClip(Thread.currentThread().getContextClassLoader().getResource("saucer.au"));
        clipTotal++;
        thrustersSound = getAudioClip(Thread.currentThread().getContextClassLoader().getResource("thrusters.au"));
        clipTotal++;
        warpSound = getAudioClip(Thread.currentThread().getContextClassLoader().getResource("warp.au"));
        clipTotal++;
    ...
    private void updateUfo() {
        ...
        if (ufo.active) {
            ...
            if (ufo.active) {
                ...
                // too lose to it.
                int d = (int) Math.max(Math.abs(ufo.x - ship.x), Math.abs(ufo.y - ship.y));
                if (ship.active && hyperCounter <= 0 && ufo.active && !missle.active && d > MAX_ROCK_SPEED * FPS / 2
    ...
    private void explode(AsteroidsSprite s) {
