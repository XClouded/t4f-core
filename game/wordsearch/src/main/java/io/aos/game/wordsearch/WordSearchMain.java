 ...
 * under the License.                                           *
 ****************************************************************/
package io.aos.game.wordsearch;
...
class WSGrid {
  ...
  public void fill() {
    ...
    this.clear();
    for (i = 0; i < WordSearchMain.words.size(); i++) {

      // Get word and pick a random starting position.

      ws = (WSWord) WordSearchMain.words.elementAt(i);
      ...
      // If the word couldn't be added, output a console message and remove it
      // from the list.

      if (!added) {
        System.out.println("Word '" + ws.list
          + "' dropped, too long or no suitable space left.");
        WordSearchMain.words.removeElementAt(i);
        i -= 1;
      }
    }

    // Fill in the rest of grid with random letters.
  ...
  public void draw(Graphics g) {
    ...
    // Shade and circle the current selection, if any.

    if (WordSearchMain.words.size() > 0 && this.select) {
  ...
  private Polygon getPolygon(int x1, int y1, int x2, int y2) {
    ...
    // Build the polygon as a half circle around each endpoint with those half
    // circles connected.
...
class WSList {
  ...
  public void draw(Graphics g) {
    ...
    g.setFont(this.font);
    fm = g.getFontMetrics();
    this.scroll = Math.min(Math.max(this.scroll, 0), Math.max(WordSearchMain.words.size() - this.height / fm.getHeight(), 0));
    x = this.x + fm.getMaxAdvance() / 2;
    y = this.y + fm.getHeight();
    for (i = this.scroll; i < WordSearchMain.words.size() && y < this.y + this.height; i++) {
      ws = (WSWord) WordSearchMain.words.elementAt(i);
      if (ws.found)
        g.setColor(this.fdColor);
      else
...
public class WordSearchMain extends Applet
